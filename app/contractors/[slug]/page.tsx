...
export async function generateMetadata({
  ...
  const data = await getContractorBySlug(slug);
  if (!data) return { title: "Contractor not found" };
  const { summary } = data;
  const title = `${summary.name} — Calgary Building Permits`;
  const description = `${summary.name} has pulled ${fmtNum(summary.permitCount)} City of Calgary building permits worth ${fmtCurrency(summary.totalCost)} total. Browse every project, address, and permit type.`;
  return {
    title,
    description,
    alternates: { canonical: `/contractors/${slug}` },
    ...
    <PageShell>
      <div className="max-w-5xl mx-auto">
        <nav className="text-sm text-zinc-500 dark:text-zinc-400 mb-6 fade-up">
          <Link href="/contractors" className="hover:underline">Contractors</Link>
        ...
        <p className="fade-up delay-2 text-base sm:text-lg text-zinc-600 dark:text-zinc-400 max-w-2xl mb-8 sm:mb-10">
          Every City of Calgary building permit filed by {summary.name} from{" "}
          {fmtDate(summary.firstDate)} to {fmtDate(summary.lastDate)}.
        </p>

        <div className="fade-up delay-3 grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10">
          ...
          <Stat label="Active since" value={summary.firstDate?.slice(0, 4) ?? "—"} />
        </div>
                ...
                <div className="flex items-start justify-between gap-3 sm:gap-4">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
                      {[p.communityname, p.permittype, p.workclass].filter(Boolean).join(" · ")}
                    </div>
                    {p.description && (
        ...
        <div className="glass rounded-2xl p-6 text-center">
          <p className="text-sm text-zinc-600 dark:text-zinc-400 mb-3">
            Want deeper analysis — every project, every year, compared to other contractors?
          </p>
          <Link
            href={`/?q=${encodeURIComponent(`all permits by ${summary.name} grouped by year`)}`}
